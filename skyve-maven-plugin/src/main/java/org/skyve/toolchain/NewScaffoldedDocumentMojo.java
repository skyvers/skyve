...
public class NewScaffoldedDocumentMojo extends NewDocumentMojo {
    ...
    private void createExtensionClass() {
        ...
        final JavaFile javaFile = JavaFile.builder("modules." + moduleName + "." + documentName, documentExtension)
                .indent("\t")
                .build();
    ...
    private void createBizletClass() {
        ...
        final TypeSpec documentBizlet = TypeSpec.classBuilder(bizletName)
                .addModifiers(Modifier.PUBLIC)
                .superclass(ParameterizedTypeName.get(ClassName.get(Bizlet.class), ClassName.get("modules." + moduleName + "." + documentName, getExtensionName())))
                .build();

        final JavaFile javaFile = JavaFile.builder("modules." + moduleName + "." + documentName, documentBizlet)
                .indent("\t")
                .build();
    ...
    private void createFactoryClass() {
        ...
        final MethodSpec crudInstance = MethodSpec.methodBuilder("crudInstance")
                ...
                .addStatement("return new $T().build($T.MODULE_NAME, $T.DOCUMENT_NAME)", DataBuilder.class, extensionClassName, extensionClassName)
                .build();
        ...
        final JavaFile javaFile = JavaFile.builder("modules." + moduleName + "." + documentName, documentFactory)
                .indent("\t")
                .build();
    ...
    private void createServiceClass() {
        ...
        final MethodSpec get = MethodSpec.methodBuilder("get")
                ...
                .addStatement("final $T query = persistence.newDocumentQuery($T.MODULE_NAME, $T.DOCUMENT_NAME)", DocumentQuery.class, extensionClassName, extensionClassName)
        ...
        final TypeSpec serviceClass = TypeSpec.classBuilder(serviceName)
                .addJavadoc(CodeBlock.builder().add("This class acts as a service layer to encapsulate domain logic.\n\n").add("Add this line to classes that wish to use it: @Inject private transient " + serviceName + " service;").build())
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(AnnotationSpec.builder(ClassName.get(Default.class)).build())
                .addField(FieldSpec.builder(Persistence.class, "persistence").addAnnotation(Inject.class).addModifiers(Modifier.PRIVATE).build())
                .addMethod(get)
                .addMethod(getAll)
                .build();

        final JavaFile javaFile = JavaFile.builder("modules." + moduleName + "." + documentName, serviceClass)
                .indent("\t")
                .build();
    ...
    private void generateDomain() throws MojoExecutionException {
        ...
        try {
            configureClasspath();
            final AbstractRepository repository = new LocalDesignRepository(srcDir, false);
