...
public class NewActionMojo extends AbstractSkyveMojo {
		...
		try {
			...
			if (! documentDirectory.exists()) {
				throw new MojoExecutionException(String.format("Directory %s for new document %s does not exist.",
																documentDirectory.getAbsolutePath(),
																documentName));
			}
			...
			if (! actionsDirectory.exists()) {
				if (! actionsDirectory.mkdir()) {
					throw new MojoExecutionException(String.format("Failed to create actions directory %s for new action %s.",
																	actionsDirectory.getAbsolutePath(),
																	actionName));
				}
			}
			createActionClass();
	...
	private void createActionClass() throws IOException {
		...
		final TypeName documentClass = extensionClassExists ?
										ClassName.get("modules." + moduleName + "." + documentName, extensionName) :
										ClassName.get("modules." + moduleName + ".domain", documentName);
		...
		final JavaFile javaFile = JavaFile.builder("modules." + moduleName + "." + documentName + ".actions", actionClass).indent("\t").build();
