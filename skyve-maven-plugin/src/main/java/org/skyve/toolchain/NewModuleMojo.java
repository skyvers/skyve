        ...
        try {
            ...
            if (newModuleFile.exists()) {
                throw new MojoExecutionException(String.format("Directory %s for new module %s already exists.",
                        newModuleFile.getAbsolutePath(), newModuleName));
            }
            ...
            try {
                ...
                final File customerFile = Paths.get(customerDirectory.getAbsolutePath(), String.format("%s.xml", customerName)).toFile();

                if (!customerFile.exists()) {
                    LOGGER.warn("Customer metadata {} does not exist, module will need to be added to customer metadata manually.", customerFile.getAbsolutePath());
                    return;
                }
                ...
                LOGGER.info("Successfully added module {} to customer metadata {}.", newModuleName, customerFile.getAbsolutePath());
