...
public class WildcatDoclet {
	...
	public static boolean start(RootDoc root) {
		root.printNotice("Wildcat Doclet, running the standard doclet");
		Standard.start(root);
		root.printNotice("Wildcat doclet, altering javadocs");
		try {
	...
	private static void generatePackageDiagrams(RootDoc root, AccessibleOptions opt,
		...
		for (ClassDoc classDoc : root.classes()) {
			...
			if (!packages.contains(packageDocName)) {
				...
				OptionProvider view = new WildcatPackageView(outputFolder, packageDoc,
	...
	private static void generateContextDiagrams(RootDoc root, AccessibleOptions opt,
		...
		for (ClassDoc classDoc : classDocs) {
			AccessibleOptions newOpt = opt;
			view = new WildcatContextView(outputFolder, classDoc, root, newOpt.getOptions());
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(new FileInputStream(htmlFile), 
			...
			while ((line = reader.readLine()) != null) {
				...
				if (!matched && insertPointPattern.matcher(line).matches()) {
					...
					writer.write("<!-- UML diagram added by Wildcat doclet -->");
					writer.newLine();
					writer.write(tag);
					writer.newLine();
				}
