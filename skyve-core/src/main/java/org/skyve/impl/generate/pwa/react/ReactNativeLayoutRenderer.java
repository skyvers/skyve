...
import org.skyve.impl.generate.pwa.LayoutRenderer;
...
public class ReactNativeLayoutRenderer extends LayoutRenderer {
	...
	public ReactNativeLayoutRenderer(Map<String, String> imports) {
	...
	public RenderedComponent tabLayout(RenderedComponent component) {
		imports.put("{ View }", ReactNativeViewRenderer.REACT_NATIVE_IMPORT);
		RenderedComponent result = new RenderedComponent();
		StringBuilder output = result.getOutput();
		output.append("<View>");
		result.setAfter("</View>");
		return result;
	}
