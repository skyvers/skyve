...
class NavigateCalendarTest {

	private NavigateCalendar step;

	@BeforeEach
	void setUp() {
		step = new NavigateCalendar();
	}

	@Test
	void defaultStartBindingIsNull() {
		assertThat(step.getStartBinding(), is(nullValue()));
	}

	@Test
	void defaultEndBindingIsNull() {
		assertThat(step.getEndBinding(), is(nullValue()));
	}

	@Test
	void setStartBindingRoundtrip() {
		step.setStartBinding("startDate");
		assertThat(step.getStartBinding(), is("startDate"));
	}

	@Test
	void setStartBindingTrimsWhitespace() {
		step.setStartBinding("  startDate  ");
		assertThat(step.getStartBinding(), is("startDate"));
	}

	@Test
	void setEndBindingRoundtrip() {
		step.setEndBinding("endDate");
		assertThat(step.getEndBinding(), is("endDate"));
	}
	...
	void executeCallsExecutor() {
		...
		verify(executor).executeNavigateCalendar(step);
	}
