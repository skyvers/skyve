...
class ZoomOutTest {
	...
	void executeCallsExecutor() {
		...
		verify(executor).executeZoomOut(step);
	}

	@Test
	void getIdentifierReturnsZoomOut() {
		AutomationContext<?, ?> ctx = mock(AutomationContext.class);
		assertThat(step.getIdentifier(ctx), is(ImplicitActionName.ZoomOut.toString()));
	}
}
