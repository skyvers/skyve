...
class DataGridRemoveTest {

	private DataGridRemove step;

	@BeforeEach
	void setUp() {
		step = new DataGridRemove();
	}
	...
	void executeCallsExecutor() {
		...
		verify(executor).executeDataGridRemove(step);
	}

	@Test
	void getIdentifierReturnsDotRemove() {
		step.setBinding("contacts");
		AutomationContext<?, ?> ctx = mock(AutomationContext.class);
		assertThat(step.getIdentifier(ctx), is("contacts.remove"));
	}
}
