...
class DataGridZoomTest {

	private DataGridZoom step;

	@BeforeEach
	void setUp() {
		step = new DataGridZoom();
	}
	...
	void setRowRoundtrip() {
		step.setRow(Integer.valueOf(2));
		assertThat(step.getRow(), is(Integer.valueOf(2)));
	}
	...
	void executeCallsExecutor() {
		...
		verify(executor).executeDataGridZoom(step);
	}

	@Test
	void getIdentifierReturnsDotZoom() {
		step.setBinding("contacts");
		AutomationContext<?, ?> ctx = mock(AutomationContext.class);
		assertThat(step.getIdentifier(ctx), is("contacts.zoom"));
	}
}
