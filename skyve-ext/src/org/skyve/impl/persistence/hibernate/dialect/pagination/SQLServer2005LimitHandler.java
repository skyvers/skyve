package org.skyve.impl.persistence.hibernate.dialect.pagination;
...
public class SQLServer2005LimitHandler extends AbstractLimitHandler {
	...
	public String getProcessedSql() {
		...
		if ( LimitHelper.hasFirstRow( selection ) ) {
			...
			sb.append( "WHERE __hibernate_row_nr__ > ").append(selection.getFirstRow()).append(" AND __hibernate_row_nr__ < ").append(getMaxOrLimit());
		}
		else {
			hasOffset = false;
			addTopExpression( sb );
	 ...
	 */
	private static boolean selectsMultipleColumns(String expression) {
	 ...
	 * @return Column alias.
	 */
	private static String getAlias(String expression) {
	 ...
	 */
	@SuppressWarnings("static-method")
	protected void encloseWithOuterQuery(StringBuilder thisSql) {
		thisSql.insert( 0, "SELECT inner_query.*, ROW_NUMBER() OVER (ORDER BY CURRENT_TIMESTAMP) as __hibernate_row_nr__ FROM ( " );
		thisSql.append( " ) inner_query " );
	}
	 ...
	 * @param sql SQL query.
	 */
	protected void addTopExpression(StringBuilder thisSql) {
		final int distinctStartPos = shallowIndexOfWord( thisSql, DISTINCT, 0 );
		StringBuilder topExpression = new StringBuilder(16);
		topExpression.append(" TOP(").append(getMaxOrLimit() - 1).append(')');
		if ( distinctStartPos > 0 ) {
			// Place TOP after DISTINCT.
			thisSql.insert( distinctStartPos + DISTINCT.length(), topExpression);
		}
		else {
			final int selectStartPos = shallowIndexOf( thisSql, SELECT_WITH_SPACE, 0 );
			// Place TOP after SELECT.
			thisSql.insert( selectStartPos + SELECT.length(), topExpression);
		}
		topAdded = true;
	}
