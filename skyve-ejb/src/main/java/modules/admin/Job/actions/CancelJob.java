...
public class CancelJob implements ServerSideAction<Job> {
    ...
    public ServerSideActionResult<Job> execute(Job bean, WebContext webContext) throws Exception {
        ...
        String triggerName = bean.getTriggerName();

        try {
            boolean result = JobScheduler.cancelJob(triggerName);
            if (result) {
                // Cancelled job
            ...
            // cancel() a UnableToInterruptJobException will be thrown
            // with the reason contained
