package modules.admin.CommunicationManager;
...
import modules.admin.domain.CommunicationManager;

public class CommunicationManagerExtension extends CommunicationManager {
	...
	private static final String COMMUNICATIONS_BATCH_PREFIX = "CommunicationManager_";
	private static final String ZIP_NAME_FORMAT = "yyyyMMddHHmmss";

	/**
	 * Construct the name of the Communication management folder
	 * 
	 * @return
	 */
	public static String getBasePath() {
		...
		sb.append(COMMUNICATIONS_BATCH_PREFIX).append(CORE.getUser().getCustomerName());
		return sb.toString();
	}
	...
	public static String getZipName() {
		...
		zipName.append(COMMUNICATIONS_BATCH_PREFIX).append(sdf.format(new DateTime())).append(".")
	...
	public void marshallCommunicationBean(final Communication communication, final String name) {
		// marshall each Communication to a JSON string
		String json = JSON.marshall(CORE.getCustomer(), communication);
		byte[] jsonBytes = json.toString().getBytes(Charset.forName("UTF-8"));

		// save the json string to a file
		StringBuilder communicationFileName = new StringBuilder(getPathToZip());
		communicationFileName.append(File.separator).append(FileUtil.safeFileName(name)).append(".")
				.append(MimeType.json.getStandardFileSuffix());

		File targetFile = new File(communicationFileName.toString());
		try (OutputStream outStream = new FileOutputStream(targetFile)) {
			...
			throw new ValidationException(String.format("The Communication %s could not be written.", name));
		}
