...
public class SkyvePanelRenderer extends PanelRenderer {
	...
	public void encodeBegin(FacesContext context, UIComponent component) throws IOException {
		...
		String collapsed 	= context.getExternalContext().getSessionMap().get(panelUniqueName) + "";
		
		if (collapsed != null) {
			panel.setCollapsed(Boolean.parseBoolean(collapsed));
		}
		
		// Saving the state of the panel toggle into the session when the user toggle the panel by this ajax to call the decode method
	...
	public void decode(FacesContext context, UIComponent component) {
		...
		{
			context.getExternalContext().getSessionMap().put(panelUniqueName, collapsedParam);
		}
