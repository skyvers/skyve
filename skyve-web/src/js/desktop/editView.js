...
isc.EditView.addMethods({
  ...
  initWidget() {
    ...
    this._vm = isc.ValuesManager.create({
      ...
      saveEditorValues(values, saveOperation, callback, context) {
   ...
   */
  _rerenderBlurryAction(validate, source) {
    this._source = source;
    this.saveInstance(validate, null, () => {
      if (this._blurry) {
   ...
   */
  _doBlurryAction(action, validate) {
   ...
   */
  _createIconMarkup(iconFile, fontIconClass) {
    if (iconFile) {
   ...
   */
  _createHelpMarkup(helpFile, helpURL) {
    if (helpFile) {
   ...
   */
  _handleListMembership(data, valueMaps, gridBinding, grid) {
    const candidates = [];
   ...
   * @param {Object} grid - the grid object to be updated.
   */
  _handleDataGrid(data, valueMaps, gridBinding, grid) {
   ...
   */
  _processDateTimeFields(data, name, isDate, isTime) {
    data.forEach((row) => {
   ...
   * @param {Object} grid - the grid object to be updated.
   */
  _handleGridSelection(data, grid) {
   ...
   */
  _manageActionPanel(values, onlyView) {
   ...
   */
  _handleToolVisibility(tool, values, onlyView) {
    if (this._b) {
      this._showHide(
        tool,
        this._actionPanel,
   ...
   */
  _displayGrowlsAndMessages(growls, messages) {
    if (growls) {
   ...
   */
  refreshListGrids(forceRefresh, forcePostRefresh, values) {
    if (forceRefresh) {
   ...
   */
  _updateValueMap(item, values, valueMaps) {
    let valueMap = {};
   ...
   */
  _enableDisable(widget, parent, values) {
   ...
   */
  _setDisabled(widget, parent, disabledConditionName, values) {
   ...
   */
  setDisabled(binding, disabledConditionName) {
   ...
   */
  _showHide(widget, parent, values, forceInvisible) {
   ...
   * @param {string} invisibleConditionName - the condition to evaluate for widget invisibility.
   */
  setInvisible(binding, invisibleConditionName) {
    const widget = this._vm.getItem(binding);
...
isc.BizDynamicImage.addMethods({
   ...
   */
  _zoom(zoomLevel, throttle = false) {
    if (zoomLevel > 0) {
   ...
   */
  _generateImageSrc(width, height, contextValue, baseValue) {
   ...
   */
  _createImageElement(width, height, src) {
...
isc.BizChart.addClassMethods({
  /**
   * Indicates whether Chart.js is currently being loaded.
   * @type {boolean}
   */
  ...
  /**
   * A counter to generate unique IDs for chart instances.
   * @type {number}
   */
  v: 0,
});
