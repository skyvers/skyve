...
isc.BizListGrid.addMethods({
  ...
  initWidget(config) {
    ...
    const me = this; // Required for inner methods that require access to parent and own 'this' context
  ...
  _createGrid(config, fields, canExpandRecords) {
    ...
    const gridConfig = {
      ...
      filterData(criteria, callback, requestProperties) {
        let result = criteria || {};
        requestProperties = requestProperties || { params: {} };
   ...
   */
  setLookup(lookup, filterParams, view) {
    this._lookup = lookup;
  ...
  setDataSource(ID, menuConfig) {
    ...
    if (this.isRepeater || this.aggregate) {
      ...
      fields.add({
        ...
        hoverHTML: (record) => record.bizFlagComment,
      });
    }
   ...
   */
  zoom(zoomToNew, contConv, newParams) {
