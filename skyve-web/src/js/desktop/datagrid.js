      ...
      click: () => {
        isc.ask("Do you want to delete/remove the selected rows?", (value) => {
          if (value) {
            ...
            this.grid.removeSelectedData((dsResponse) => {
...
isc.BizDataGrid.addMethods({
  ...
  _createGrid(config, fields, contextMenu) {
    ...
    const gridConfig = {
      ...
      recordsDropped(dropRecords, index) {
        this.reorderData();
      },
      reorderData() {
        if (config._ordinal) {
        ...
        if (config.editable && record && !record.isFolder) {
          ...
          if (config.inline) {
            if (me.canUpdate && me.canEdit && me.showEdit && !me._disabled) {
              me.startEditing(me._eventRowNum, me._eventColNum, false);
            }
      ...
      editComplete(rowNum, colNum) {
        me._view._vm.setValue("_changed", true);
        me._view._vm.setValue("_apply", true);
        if (me.bizEdited) me.bizEdited();
      },
    };
