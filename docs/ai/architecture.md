...
Global framework services (mail, persistence helpers, dispatch decorators, etc.) are effectively shared infrastructure and should be designed as such:

- initialise and compose them during deterministic startup paths
- keep request-time access simple and side-effect free
- treat lazy initialisation as an explicit concurrency decision, not a convenience default
- fail fast on use-before-initialisation when startup wiring is a required invariant
- avoid rebuilding equivalent decorator chains on hot paths when composition can be done once at bootstrap

These rules keep service behaviour predictable under concurrency and reduce hidden runtime costs.
