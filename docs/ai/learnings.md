...
- Concurrency is a contract, not an implementation detail. For globally shared services, document who initialises them, when they are visible to other threads, and whether access should fail-fast before initialisation completes.
- If reviewers cannot infer "why this exists," treat that as a design bug. Add explicit intent near the code (Javadoc, invariants, or requirement notes), especially for sanitisation, pre-processing, and logging behaviour.
...
- Immutable API boundaries are useful, but repeated full-object copy construction can become hidden overhead. When adapting immutable inputs, prefer one-pass transforms, shared builders, or SPI-level mutability where safe and well-contained.
