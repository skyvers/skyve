...
public class MyUserLoginHistoryModel extends ChartModel<DashboardExtension> {
	...
	public ChartData getChartData() {
		...
		return pers.withDocumentPermissionScopes(DocumentPermissionScope.customer, p -> {
			...
			q.getFilter()
					.addGreaterThan(UserLoginRecord.loginDateTimePropertyName, new DateOnly(LocalDateTime.now().minusMonths(1))); // Records from the past month
			...
			ChartData chartData = cb.build("My login history - last 14 days", "Login Count");
			return chartData;
		});
	}
}
