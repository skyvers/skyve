package modules.admin.Dashboard.chain.services;
...
/**
 * Service for creating dashboard favourites widgets based on audit history.
 */
@Default
public class FavouritesService {
	 ...
	 * 
	 * @param dashboard The dashboard to populate with favourites
	 * @param persistence1 The persistence context
	 */
	public void createFavourites(DashboardExtension dashboard, Persistence persistence1) {
		...
		this.persistence.withDocumentPermissionScopes(DocumentPermissionScope.customer, p -> {
			...
			if (this.tiles.size() < TILE_COUNT_LIMIT) {
				...
				Customer customer = this.persistence.getUser().getCustomer();
		...
		for (Tile tile : this.tiles) {
			...
			dashboard.addFavouritesElement(g);
		}
	}
