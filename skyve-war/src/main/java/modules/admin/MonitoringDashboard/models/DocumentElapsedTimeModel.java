...
public class DocumentElapsedTimeModel extends ChartModel<MonitoringDashboard> {
	...
	public ChartData getChartData() {
		...
		cd.setTitle("Create Request Elapsed Time - " + selectedDocument);
		...
		Map<Integer, Integer> createData = null;
		String timePeriod = "hours";
		
		if (createMeasurements != null) {
			createData = createMeasurements.getHoursMillis();
			if (createData.isEmpty()) {
				createData = createMeasurements.getMinutesMillis();
				timePeriod = "minutes";
				if (createData.isEmpty()) {
					createData = createMeasurements.getSecondsMillis();
					timePeriod = "seconds";
				}
			}
		}
		...
		cd.setTitle("Create Request Elapsed Time - " + selectedDocument + " (" + getTimePeriodLabel(timePeriod) + ")");
	...
	private void buildTimeSeriesData(List<String> timeLabels, List<Number> createValues, 
									Map<Integer, Integer> createData, String timePeriod) {
		...
		for (int i = 0; i <= maxIndex; i++) {
			...
			Integer createValue = (createData != null) ? createData.get(i) : null;
