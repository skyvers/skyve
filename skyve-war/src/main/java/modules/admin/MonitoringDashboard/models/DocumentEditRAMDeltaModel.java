...
public class DocumentEditRAMDeltaModel extends ChartModel<MonitoringDashboard> {
	...
	public ChartData getChartData() {
		...
		cd.setTitle("Edit Request RAM Delta - " + selectedDocument);
		...
		Map<Integer, Number> editData = null;
		String timePeriod = "hours";
		
		if (editMeasurements != null) {
			editData = editMeasurements.getHoursRAMPercentageDelta();
			if (editData.isEmpty()) {
				editData = editMeasurements.getMinutesRAMPercentageDelta();
				timePeriod = "minutes";
				if (editData.isEmpty()) {
					editData = editMeasurements.getSecondsRAMPercentageDelta();
					timePeriod = "seconds";
				}
			}
		}
		...
		cd.setTitle("Edit Request RAM Delta - " + selectedDocument + " (" + getTimePeriodLabel(timePeriod) + ")");
	...
	private void buildTimeSeriesData(List<String> timeLabels, List<Number> editValues, 
									Map<Integer, Number> editData, String timePeriod) {
		...
		for (int i = 0; i <= maxIndex; i++) {
			...
			Number editValue = (editData != null) ? editData.get(i) : null;
