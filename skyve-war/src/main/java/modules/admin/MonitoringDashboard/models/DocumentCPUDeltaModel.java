...
public class DocumentCPUDeltaModel extends ChartModel<MonitoringDashboard> {
	...
	public ChartData getChartData() {
		...
		cd.setTitle("Create Request CPU Delta - " + selectedDocument);
		cd.setLabel("CPU Delta (cores)");
		...
		Map<Integer, Float> createData = null;
		String timePeriod = "hours";
		
		if (createMeasurements != null) {
			createData = createMeasurements.getHoursCPUCoresDelta();
			if (createData.isEmpty()) {
				createData = createMeasurements.getMinutesCPUCoresDelta();
				timePeriod = "minutes";
				if (createData.isEmpty()) {
					createData = createMeasurements.getSecondsCPUCoresDelta();
					timePeriod = "seconds";
				}
			}
		}
		...
		cd.setTitle("Create Request CPU Delta - " + selectedDocument + " (" + getTimePeriodLabel(timePeriod) + ")");

		// Set up line chart data with create requests as primary series
	...
	private void buildTimeSeriesData(List<String> timeLabels, List<Number> createValues, 
									Map<Integer, Float> createData, String timePeriod) {
		...
		for (int i = 0; i <= maxIndex; i++) {
			...
			Float createValue = (createData != null) ? createData.get(i) : null;
			createValues.add((createValue != null) ? createValue : 0.0f);
		}
