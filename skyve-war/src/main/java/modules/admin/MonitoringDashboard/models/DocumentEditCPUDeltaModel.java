...
public class DocumentEditCPUDeltaModel extends ChartModel<MonitoringDashboard> {
	...
	public ChartData getChartData() {
		...
		cd.setTitle("Edit Request CPU Delta - " + selectedDocument);
		...
		Map<Integer, Float> editData = null;
		String timePeriod = "hours";
		
		if (editMeasurements != null) {
			editData = editMeasurements.getHoursCPUCoresDelta();
			if (editData.isEmpty()) {
				editData = editMeasurements.getMinutesCPUCoresDelta();
				timePeriod = "minutes";
				if (editData.isEmpty()) {
					editData = editMeasurements.getSecondsCPUCoresDelta();
					timePeriod = "seconds";
				}
			}
		}
		...
		cd.setTitle("Edit Request CPU Delta - " + selectedDocument + " (" + getTimePeriodLabel(timePeriod) + ")");
	...
	private void buildTimeSeriesData(List<String> timeLabels, List<Number> editValues, 
									Map<Integer, Float> editData, String timePeriod) {
		...
		for (int i = 0; i <= maxIndex; i++) {
			...
			Float editValue = (editData != null) ? editData.get(i) : null;
			editValues.add((editValue != null) ? editValue : 0.0f);
		}
