...
isc.FormItem.addMethods({
    ...
    redrawing : function () {
        ...
        if (isc.Browser.isIE && this.form && !this.form._setValuesPending) {
    ...
    _showValueAfterDraw : function (redrawing) {
        ...
        if (this.isSetToDefaultValue() || (this._value == null)) this.clearValue();
    ...
    mapValueToDisplay : function (value) {
        ...
        if (this.multiple && isc.isAn.Array(value)) {
            ...
            for (var i = 0, length = value.length; i < length; i++) {
                ...
                if (key && key.trim) key = value[i] = key.trim();
    ...
    _formatDataType : function (value, applyStaticTypeFormat) {
        ...
        } else if (this._simpleType && this._simpleType.editFormatter) {
            var form = this.form,
                record = this.form ? this.form.values : {};
    ...
    getType : function () {
        ...
        if (this.criteriaField && this.form && this.form.dataSource) {
            ...
            var criteriaField = ds ? ds.getField(this.criteriaField) : null;
    ...
    fetchMissingValueReply : function (response, data, request) {
        ...
        for (var i = 0, len = newValue.length; i < len; ++i) {
            ...
            if (!fetchingMissingValues || !newValue || !newValue[i]) {
                this.logWarn("BAD VALUES: " + this.echo(clientContext));
            }
...
*/
                                         ...
                                         notFoundCount, notFoundValues, wasValueFieldFetch) 
    {
        ...
        if (this._showingLoadingDisplayValue) {
            if (!wasValueFieldFetch) {
                this.logInfo(
                    "fetchMissingDisplayFieldValueReply() - clearing loadingDisplayValue " +
                    "even though this fetch was for the user-entered display field " +
                    "(assuming overlapping fetches)", "loadingDisplayValue");
    ...
    _fetchMissingValueInProgress : function (checkDisplayFieldValues) {
        if (this._fetchingMissingValueFieldValues && 
            !isc.isAn.emptyObject(this._fetchingMissingValueFieldValues))
        {
            return true;
        }
        if (checkDisplayFieldValues && this._fetchingMissingDisplayFieldValues && 
            !isc.isAn.emptyObject(this._fetchingMissingDisplayFieldValues))
        {
            return true;
        }
        return false;
    ...
    _fetchMissingValueFieldValueReply : function (dsResponse, data, dsRequest, notFoundCount, notFoundValues) {
        ...
        this._updateValue(dsResponse.internalClientContext.newValue);
    },
    ...
    _setElementValue : function (newValue, dataValue) {
        if (this.multiple && newValue != null && isc.isAn.Array(newValue)) {
            ...
            for (var i = 0, len = newValue.length; i < len; i++) {
                ...
                // trim keys to avoid including spaces from the multipleValueSeparator
                if (newValue[i] && newValue[i].trim) newValue[i] = newValue[i].trim();
    ...
    getElementValue : function () {
        ...
        if (this._showingInFieldHint) return this.emptyDisplayValue;
    ...
    showCompletion : function (value) {
        ...
        if (this.getClass().isMethodSupported(this._$setSelectionRange)) {
    ...
    acceptCompletion : function (cursorAtEnd) {
        ...
        if (this.autoCompleteEnabled()) {
            ...
            if (this.getElementValue() != completion) {
                if (!cursorAtEnd && this.getClass().isMethodSupported(this._$getSelectionRange)) {
                    selectionRange = this.getSelectionRange();
                }
                this.setElementValue(completion);
            }

            if (this.hasFocus && selectionRange && this.getClass().isMethodSupported(this._$setSelectionRange)) {
    ...
    rememberSelection : function (timeCritical) {
        ...
        if (!this.isDrawn() || !this.getClass().isMethodSupported(this._$getSelectionRange)) return;
    ...
    resetToLastSelection : function (dataTransform) {
        ...
        if (shouldReset && this.getClass().isMethodSupported(this._$setSelectionRange)) {
    ...
    focusInItem : function () {
        ...
        if (refocusAfterRedraw) selectOnFocus = false;
    ...
    shouldStopKeyPressBubbling : function (keyName, characterValue) {
        ...
        if (this.stopCharacterKeyPressBubbling && 
        ...
        if (this.stopNavKeyPressBubbling && !isc.EH.altKeyDown()) {
            if (this._formItemNavBackKeys[keyName]) {
                if (!this.getClass().isMethodSupported(this._$getSelectionRange)) return true;
    ...
    _rememberPreChangeSelection : function () {
        if (!this.getClass().isMethodSupported(this._$getSelectionRange)) return;
