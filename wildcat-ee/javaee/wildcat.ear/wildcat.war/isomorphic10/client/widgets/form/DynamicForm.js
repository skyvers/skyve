...
isc.DynamicForm.addProperties({
    ...
    // The string pre-pended to the title of every item in this form.  See also
    // +{requiredTitlePrefix} for fields that are required.
    // @group formTitles
    // @visibility external
    //<
	titlePrefix:"",
	...
	if (!this._setValuesPending) {
        ...
        for (var i = 0; i < items.length; i++) {
            if (isc.isA.PasswordItem(items[i])) {
                ...
                if (items[i] == focusItem && focusItem._itemValueIsDirty()) {
...
_getEventTargetItemInfo : function (event) {
    ...
    // item the event occurred over directly
    event.itemInfo = info;
    return info;
},
